...
const CreatePost = ({ open, setOpen }) => {
  ...
  const createPostHandler = async (e) => {
    ...
    try {
      setLoading(true);
      const res = await axios.post('https://uno-il5c.onrender.com/api/v1/post/addpost', formData, {
        headers: {
  ...
  return (
    ...
    <Dialog  open={open} >
      <DialogContent onInteractOutside={() => setOpen(false)}>
        <DialogHeader className='text-center text-black font-semibold'>Create New Post</DialogHeader>
        <div className='flex gap-3  items-center'>
          ...
          <div>
            <h1 className='font-semibold text-black text-xs'>{user?.username}</h1>
